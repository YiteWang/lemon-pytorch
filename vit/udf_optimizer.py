...
#         # if (not param.requires_grad) or ('weight_dof' in name):
#         if 'weight_dof' in name:
#             continue
        
#         if large_patch_lr and 'patch_embed' in name:
#             large_lr.append(param)
#             continue

#         if name in no_weight_decay_list:
#             no_decay.append(param)
#         elif param.ndim <= 1 or name.endswith(".bias"):
#             others.append(param)
#         # elif any(x in name for x in add_params_list):
#             # add.append(param)
#         else:
#             decay.append(param)
        
#     if no_weight_decay_lr is None:
#         out = [
#             {'params': no_decay + others, 'weight_decay': 0.},
#             {'params': decay, 'weight_decay': weight_decay}]
#     else:
#         out = [
#             {'params': no_decay, 'weight_decay': 0., 'lr': no_weight_decay_lr},
#             {'params': others, 'weight_decay': 0.,},
#             {'params': decay, 'weight_decay': weight_decay,}]
#     # if add:
#         # out.append({'params': add, 'weight_decay': weight_decay, 'lr': add_params_lr})
#     return out
...
#         # if (not param.requires_grad) or ('weight_dof' in name):
        if fix_patch:
            if 'patch_embed' in name:
                print('Exclude patch embedding')
                continue
        if 'weight_dof' in name:
            continue
        if large_patch_lr:
            # if ('cls_token' in name) or ('pos_embed' in name) or ('patch_embed' in name) or ('head' in name) or ('blocks.0' in name):
            # if ('cls_token' in name) or ('pos_embed' in name) or ('patch_embed' in name) or ('norm' in name):
            # if ('patch_embed' in name) or ('head' in name):
            # if ('patch_embed' in name):
            # if ('patch_embed' in name) or ('norm' in name) or ('head' in name) or ('cls_token' in name) or ('pos_embed' in name) or ('blocks.0' in name):
            # if ('patch_embed' in name) or ('norm' in name) or ('head' in name) or ('cls_token' in name) or ('pos_embed' in name) or ('attn' in name):
            # if ('patch_embed' in name) or ('norm' in name) or ('head' in name) or ('cls_token' in name) or ('pos_embed' in name) or ('mlp' in name):
            # large_lr_blocks_idx = list(map(str, range(large_lr_blocks)))
            # if ('weight' in name) and (('mlp' in name) or ('attn' in name)):
            # mlp and attn only consits in blocks
            # if ('blocks' in name):
            # if (('mlp' in name) or ('attn' in name)) and not(any('blocks.'+val in name for val in large_lr_blocks_idx)):
            # if 'norm' in name:
            if ('head' in name) or ('patch_embed' in name) or ('norm' in name) or ('pos_embed' in name) or ('cls_token' in name):
                large_lr.append(param)
                continue
    ...
    if large_patch_lr:
        print('The length of three param groups:')
        print('large lr:{}, no_decay:{}, others:{}'.format(len(large_lr), len(no_decay), len(decay)))
        return [
            {'params': large_lr, 'weight_decay': weight_decay, 'lr':part_lr_frac*lr},
            {'params': no_decay, 'weight_decay': 0.},
            {'params': decay, 'weight_decay': weight_decay}]
    else:
        print('The length of two param groups:')
        print('no_decay:{}, others:{}'.format(len(no_decay), len(decay)))
        return [
            {'params': no_decay, 'weight_decay': 0.},
            {'params': decay, 'weight_decay': weight_decay}]
...
def optimizer_kwargs(cfg):
    ...
    if getattr(cfg, 'no_weight_decay_lr', None) is not None:
        kwargs['no_weight_decay_lr'] = cfg.no_weight_decay_lr
    if getattr(cfg, 'large_patch_lr', None) is not None:
        kwargs['large_patch_lr'] = cfg.large_patch_lr
    if getattr(cfg, 'fix_patch', None) is not None:
        kwargs['fix_patch'] = cfg.fix_patch
    if getattr(cfg, 'part_lr_frac', None) is not None:
        kwargs['part_lr_frac'] = cfg.part_lr_frac
    if getattr(cfg, 'large_lr_blocks', None) is not None:
        kwargs['large_lr_blocks'] = cfg.large_lr_blocks
    return kwargs
...
def create_optimizer_v2(
        ...
        no_weight_decay_lr: Optional[float] = None,
        large_patch_lr: Optional[bool] = None,
        fix_patch: Optional[bool] = None,
        part_lr_frac: Optional[bool] = None,
        large_lr_blocks: Optional[int] = None,
        **kwargs,
):
    """ Create an optimizer.
    ...
    if isinstance(model_or_params, nn.Module):
        ...
        if param_group_fn:
            ...
            weight_decay = 0.
        elif (weight_decay and filter_bias_and_bn) or no_weight_decay_lr:
            parameters = param_groups_weight_decay(model_or_params, weight_decay, 
                no_weight_decay_list=no_weight_decay, 
                no_weight_decay_lr=no_weight_decay_lr, 
                large_patch_lr=large_patch_lr,
                fix_patch=fix_patch, 
                lr=lr, 
                part_lr_frac=part_lr_frac,
                large_lr_blocks=large_lr_blocks,)
            weight_decay = 0.
        else:
