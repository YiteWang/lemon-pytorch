...
from preln_bert import modSimpleBertForMaskedLM
from postln_bert import SimpleBertForMaskedLMv2
from udf_sched import CosineAnnealingWarmupRestarts, get_twostage_linear_schedule_with_warmup, get_linear_nonzero_with_warmup
...
MODEL_CLASSES = {
    ...
    "postlnbert": (BertConfig, SimpleBertForMaskedLMv2, BertTokenizer),
    "modbert": (BertConfig, modSimpleBertForMaskedLM, BertTokenizer),
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    # 
    if args.offset_steps != 0:
        assert args.sched == 'udfcosine'
    ...
    if args.sched == 'cosine':
        ...
        scheduler = get_polynomial_decay_schedule_with_warmup(
            optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total, power=args.poly, lr_end=args.learning_rate * args.lr_decay,
        )
    elif args.sched == 'udfcosine':
        scheduler = CosineAnnealingWarmupRestarts(
            optimizer, warmup_steps=args.warmup_steps, 
            max_lr=args.learning_rate, min_lr=args.learning_rate * args.lr_decay,
            first_cycle_steps=t_total+args.offset_steps,
        )
    elif args.sched == 'twostagelinear':
        print('Use 2stagelinear')
        scheduler = get_twostage_linear_schedule_with_warmup(
            optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total, fast_decay_steps=args.fast_decay_steps, second_lr=args.lr_decay,
        )
    elif args.sched == 'linearnonzero':
        print('Use linearnonzero')
        scheduler = get_linear_nonzero_with_warmup(
            optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=t_total, second_lr=args.lr_decay,
        )
    else:
    ...
    if args.gpu == 0:
        logger.info(" Evaluation Results before training: ")
        results = evaluate(args, model, tokenizer)
        for key, value in results.items():
            tb_writer.add_scalar("eval_iter_{}".format(key), value, global_step)
            logger.info("\t %s: %0.4f" % (key, value))
        output_eval_file = os.path.join(args.output_dir, "eval_results_before_train.json")
        json.dump(results, open(output_eval_file, 'w'), sort_keys=True, indent=4)

    # make sure other gpus wait for gpu 0 finish evaluate
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                optimizer.step()
                if args.sched == 'udfcosine':
                    scheduler.step(global_step+args.offset_steps)  # Update learning rate schedule
                else:
                    scheduler.step()
                model.zero_grad()
                global_step += 1
                if args.gpu == 0 and args.logging_steps > 0 and (global_step + 1) % args.logging_steps == 0:
                    # Log metrics
                    tb_writer.add_scalar("lr", scheduler.get_lr()[0], global_step)
                    track_lr.append(scheduler.get_lr()[0])
                    np.save(os.path.join(args.output_dir, 'track_lr.npy'), np.array(track_lr))
                    if args.fp16:
                        try:
                    ...
                    print_loss = (tr_loss - logging_loss) / args.logging_steps
                    tb_writer.add_scalar("loss", (tr_loss - logging_loss) / args.logging_steps, global_step)
                    logging_loss = tr_loss

                    # Save it each epoch
                    if args.gpu == 0 and args.eval_when_log:

                        # Save checkpoints every 50000 iterations
                        if (global_step + 1) % (50000) == 0:
                            checkpoint_name = "checkpoint-iter{}".format(global_step)
                            save_model(args, checkpoint_name, model, tokenizer, optimizer, scheduler)

                        # Evaluate the model
                        logger.info(" Evaluation Results of iteration %d: " % global_step)
                        results = evaluate(args, model, tokenizer)
                        for key, value in results.items():
                            tb_writer.add_scalar("eval_iter_{}".format(key), value, global_step)
                            logger.info("\t %s: %0.4f" % (key, value))
                            track_eval.append(value)
                            np.save(os.path.join(args.output_dir, 'track_eval.npy'), np.array(track_eval))
                            
                        print('Time: {}, Curr iter: {}, lr:{}, Eval:{}, Loss:{}'.format(datetime.now(), 
                            global_step, scheduler.get_lr()[0], results["perplexity"], print_loss))
        ...
        if args.gpu == 0:
            ...
            json.dump(results, open(output_eval_file, 'w'), sort_keys=True, indent=4)
            print('[*] Current Epoch: {}, Evaluation perplexity:{}'.format(epoch, results["perplexity"]))
...
def setup(gpu, args):
    ...
    # Setup logging
    logging.basicConfig(
        filename=os.path.join(args.output_dir, 'warning_log.txt'),
    ...
    # Get Config
    if args.config_name:
        print('Load config from :{}'.format(args.config_name))
        config = config_class.from_pretrained(args.config_name, cache_dir=args.cache_dir)
    elif args.model_name_or_path:
        print('Load config from :{}'.format(args.model_name_or_path))
    ...
    while True:
        try:
            if args.tokenizer_name:
                print('Load tokenizer from:{}'.format(args.tokenizer_name))
